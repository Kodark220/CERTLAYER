...
class CertLayerContract(gl.Contract):
    ...
    def _require_protocol_operator(self, protocol_id: str):
        owner_wallet = self.protocol_owner_wallet.get(protocol_id, "")
        sender_wallet = self._sender_wallet()
        if sender_wallet != self.contract_admin and sender_wallet != owner_wallet:
            raise Exception("only protocol owner or admin allowed")

    def _require_incident_operator(self, incident_id: str):
        if incident_id not in self.incident_protocol_id:
            raise Exception("incident not found")
        self._require_protocol_operator(self.incident_protocol_id[incident_id])

    def _require_commitment_operator(self, commitment_id: str):
        if commitment_id not in self.commitment_protocol_id:
            raise Exception("commitment not found")
        self._require_protocol_operator(self.commitment_protocol_id[commitment_id])

    def _require_dispute_submitter(self, wallet: str):
        sender_wallet = self._sender_wallet()
        if sender_wallet != self.contract_admin and sender_wallet != wallet.lower():
            raise Exception("only disputed wallet or admin allowed")

    def _extract_protocol_id_from_payload(self, payload_json: str) -> str:
        try:
            payload = json.loads(payload_json)
        except Exception:
            raise Exception("payload_json must be valid json")
        protocol_id = str(payload.get("protocolId", ""))
        if protocol_id == "":
            raise Exception("payload_json must include protocolId")
        return protocol_id
    ...
    def register_protocol(self, protocol_id: str, metadata_json: str, owner_wallet: str, contract_address: str = ""):
        ...
        sender_wallet = self._sender_wallet()
        normalized_owner = owner_wallet.lower()
        if sender_wallet != self.contract_admin and sender_wallet != normalized_owner:
            raise Exception("owner_wallet must match sender unless sender is admin")

        self.protocol_metadata[protocol_id] = metadata_json
        self.protocol_owner_wallet[protocol_id] = normalized_owner
    ...
    def get_protocol_contract_address(self, protocol_id: str) -> str:
        if protocol_id not in self.protocol_contract_address:
            return ""
        return self.protocol_contract_address[protocol_id]
    ...
    def submit_incident_candidate(self, incident_id: str, payload_json: str):
        ...
        protocol_id = self._extract_protocol_id_from_payload(payload_json)
        if protocol_id not in self.protocol_metadata:
            raise Exception("protocol not found")
        self._require_protocol_operator(protocol_id)
    ...
    def set_last_clean_block(self, incident_id: str, block_number: int):
        ...
        self._require_incident_operator(incident_id)
    ...
    def raise_dispute(self, incident_id: str, wallet: str, evidence_hash: str):
        if incident_id not in self.incident_payload:
            raise Exception("incident not found")
        self._require_dispute_submitter(wallet)
    ...
    def evaluate_commitment(self, commitment_id: str, result: str, evidence_hash: str, current_ts: int):
        ...
        self._require_commitment_operator(commitment_id)
